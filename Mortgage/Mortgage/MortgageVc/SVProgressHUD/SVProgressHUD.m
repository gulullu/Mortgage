...
+ (void)showErrorWithStatus:(NSString *)string {
    if ([string isEqual:[NSNull null]]) {
        [self sharedView];
        [self showErrorWithStatus:@"发生未知错误,请稍后重试!" maskType:SVProgressHUDDefaultMaskType];
        return;
    }
    [self sharedView];
...
+ (void)showSoTaoImage:(UIImage*)image status:(NSString *)string maskType:(SVProgressHUDMaskType)maskType{
    
    [[self sharedView] showImage:image status:string maskType:maskType];
}
